# Ported from RTXteam/RTX @ 9485431, code/ARAX/ARAXQuery/Infer/scripts/infer_utilities.py.
# Changes from upstream:
#   - import paths / sys.path hacks only
#   - the xDTD mapping tables are read from the directory of
...
from shepherd_utils.arax.util import get_arax_edge_key
...
class InferUtilities:
    ...
    def genrete_treat_subgraphs(self, response: ARAXResponse, top_scores: pd.DataFrame, top_paths: dict, drug_curie: Optional[str], disease_curie: Optional[str], qedge_id=None):
        ...
        xdtdmapping = xDTDMappingDB(database_name=RTXConfig.explainable_dtd_db_path.split('/')[-1], mode='run', db_loc=os.path.dirname(RTXConfig.explainable_dtd_db_path))
