# Ported from RTXteam/RTX @ 9485431, code/ARAX/ARAXQuery/ARAX_query.py.
# Changes from upstream:
#   - import paths / sys.path hacks only (including class-name strings)
#   - ARAXQueryTracker and ResponseCache are Shepherd stand-ins (ARAX_query_tracker.py,
#     ResponseCache/response_cache.py): no MySQL tracker or concurrency limit, and
#     storing a response only assigns its id and envelope.id; the caller saves it.
#     ARAXQuery(response_id=...) passes the id to use (DEC-3, DEC-14)
#   - stored-response URLs are {settings.server_url}/arax/response/{id} (DEC-3): the
#     message_uris match for a local response, the 'stored with id' log line, and
...
from shepherd_utils.arax.ARAX_query_graph_interpreter import ARAXQueryGraphInterpreter
from shepherd_utils.arax.ARAX_messenger import ARAXMessenger
from shepherd_utils.arax.ARAX_ranker import ARAXRanker
from shepherd_utils.arax.operation_to_ARAXi import WorkflowToARAXi
...
from shepherd_utils.arax.ResponseCache.response_cache import ResponseCache, response_url  #noqa: E402
...
from shepherd_utils.arax.NodeSynonymizer.node_synonymizer import NodeSynonymizer  #noqa: E402
...
class ARAXQuery:
    ...
    def __init__(self, response_id=None):
        ...
        self.lock = None
        # The id the response is stored under (ResponseCache.add_new_response)
        self.store_response_id = response_id
    ...
    def query(self, query, mode='ARAX', origin='local'):
        ...
        try:
            ...
            if "have_query_graph" in query_attributes and "have_operations" not in query_attributes:
                ...
                #### Run the QueryGraph through the QueryGraphInterpreter and to generate ARAXi
    ...
    def execute_processing_plan(self,input_operations_dict, mode='ARAX'):
        ...
        response_cache = ResponseCache(self.store_response_id)  #  also calls connect
        ...
        if operations.message_uris is not None:
            ...
            for uri in operations.message_uris:
                ...
                matchResult = re.match( re.escape(response_url('')) + r'([^/?#]+)$',uri,re.M|re.I )
        ...
        if operations.messages is not None:
            ...
            for uploadedMessage in operations.messages:
                ...
                if str(uploadedMessage.__class__) == "<class 'shepherd_utils.arax.openapi_server.models.message.Message'>":
        ...
        if response.envelope.message.query_graph and response.envelope.message.query_graph.nodes and not hasattr(response, "original_query_graph"):
        ...
        if operations.actions:
            ...
            if return_action['parameters']['store'] == 'true':
                ...
                response.info(f"Result was stored with id {response_id}. It can be viewed at {response_url(response_id)}")
