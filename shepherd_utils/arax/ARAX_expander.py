# Ported from RTXteam/RTX @ 9485431, code/ARAX/ARAXQuery/ARAX_expander.py.
...
KP_THAT_CAN_HANDLE_SINGLE_NODE_QUERIES = {RETRIEVER_INFORES}
...
class ARAXExpander:
    ...
    def apply(
        ...
        if qedge_keys_to_expand:
            ...
            for qedge_key in ordered_qedge_keys_to_expand:
                ...
                if not user_specified_kp:
                    ...
                    # Query Retriever, forwarding the user's kp list (DEC-10); all others are Skipped (DEC-12)
                    kps_to_query = {RETRIEVER_INFORES}
                    for kp in kp_selector.valid_kps.difference(kps_to_query):
                        response.update_query_plan(qedge_key, kp, "Skipped", SKIPPED_NOT_RETRIEVER_MESSAGE)
        ...
        if qnode_keys_to_expand:
            kps_to_use = [RETRIEVER_INFORES]  # DEC-4: Retriever answers single-node queries
    ...
    def _expand_node(qnode_key: str,
                     ...
                     kp_timeout: Optional[int],
                     log: ARAXResponse,
                     forwarded_kps: Optional[list[str]] = None) -> QGOrganizedKnowledgeGraph:
    ...
    def _set_and_validate_parameters(self, input_parameters: dict[str, Any], kp_selector: KPSelector,
        ...
        for param_name, value in input_parameters.items():
            if param_name and param_name not in parameters:
                ...
                if param_name == "kp":
                    # DEC-10: forwarded to Retriever exactly as given; Retriever decides what is valid
                    parameters[param_name] = eu.convert_to_list(value)
    ...
    def _load_fda_approved_drug_ids() -> set[str]:
        ...
        rtxc = RTXConfiguration()
        pickle_file_path = rtxc.fda_approved_drugs_path
