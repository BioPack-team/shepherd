...
#   - connect_nodes runs with Shepherd's pathfinder limits (DEC-7): max_path_length and
#     max_pathfinder_paths are still validated as in ARAX, but the search always uses
#     4 hops and 500 paths, as the arax_pathfinder worker does. The data files come
#     from Path_Finder/utility.py (Shepherd's pathfinder downloads)
...
from shepherd_utils.arax.BiolinkHelper.biolink_helper import get_biolink_helper, get_current_arax_biolink_version
...
# Shepherd's arax_pathfinder worker's limits (NUM_TOTAL_HOPS, MAX_PATHFINDER_PATHS), DEC-7
SHEPHERD_PATHFINDER_HOPS = 4
SHEPHERD_PATHFINDER_MAX_PATHS = 500
...
def get_xcrg_retriever_url(rtx_config):
    ...
    if override:
        return override
    return settings.sync_kg_retrieval_url
...
class ARAXConnect:
    ...
    def __connect_nodes(self, describe=False):
        ...
        # DEC-7: Shepherd's pathfinder limits, whatever max_path_length / max_pathfinder_paths say
        max_pathfinder_paths = SHEPHERD_PATHFINDER_MAX_PATHS
        try:
            result, aux_graphs, knowledge_graph = pathfinder.get_paths(
                ...
                hops_numbers=SHEPHERD_PATHFINDER_HOPS,
                max_hops_to_explore=SHEPHERD_PATHFINDER_HOPS,
        ...
        if not hasattr(self.response, "original_query_graph"):
    ...
    def rehydrate(self, kg, retriever_url):
        ...
        try:
            res = requests.post(
                settings.kg_rehydrate_url, headers=headers, json=payload, timeout=30
            )
