...
tests/fixtures/ars_goldens/goldens_trapi15.json is generated by RUNNING the
pinned NCATSTranslator/Relay code over the TRAPI 1.5 corpus
tests/fixtures/ars_corpus/trapi15/ (scripts/ars_parity/generate_goldens.py).
Shepherd's ARS speaks TRAPI 2.0, so the corpus these tests run on
(tests/fixtures/ars_corpus/*.json) and the goldens they compare against
(goldens.json) are the TRAPI 2.0 translations of those files, made with
TOM's 1.6 -> 2.0 transforms (scripts/ars_parity/trapi2.py,
build_corpus.py, upconvert_goldens.py). These tests run the Shepherd ports
over the 2.0 corpus and require output identical to the translated goldens.
...
# ---------------------------------------------------------------------------
# validation verdicts (TOM TRAPI 2.0 models vs reasoner-pydantic 1.5, with
# the verdicts 2.0 itself changes declared in goldens.json _trapi2_verdicts)
# ---------------------------------------------------------------------------
...
def _validate_cases():
    ...
    # 2.0: the binding is one {"ids": [...]} object (1.5: [{"id": ...}])
    broken_binding = copy.deepcopy(aragorn)
    del broken_binding["message"]["results"][0]["node_bindings"]["sn"]["ids"]
    ...
    # pathfinder: query_graph is nodes + paths (no edges) -- 2.0's single
    # QueryGraph (edges and/or paths; 1.5 had a PathfinderQueryGraph arm)
...
def test_divergences_are_declared():
    ...
    assert set(entries) == {
        # upstream bugs the port fixes (recorded in the TRAPI 1.5 era)
        "decorate/no_primary",
        "scores/mixed",
        ...
        # TRAPI 2.0 behaviour (scripts/ars_parity/upconvert_goldens.py)
        "mergedicts/qualifier_lists_conflicting_values",
        "mergedicts/qualifier_lists_disjoint_types",
        "scores/missing_analyses_key",
        "scores/empty_analyses_list",
        "premerged_inputs.arax.message.results.1.analyses.0",
        "merge_pipeline_ab.message.results.1.analyses.0",
    }
    for name, reason in entries.items():
        assert reason.strip(), f"{name} has no reason recorded"


def test_goldens_are_the_trapi2_translation():
    """goldens.json is the TRAPI 2.0 translation of the Relay (1.5) record
    (scripts/ars_parity/upconvert_goldens.py), and every verdict 2.0 itself
    changes says why."""
    assert GOLDENS["_trapi_version"] == "2.0"
    relay = json.loads((FIXTURES / "ars_goldens/goldens_trapi15.json").read_text())
    assert relay["_relay_commit"] == GOLDENS["_relay_commit"]
    for case, reason in GOLDENS["_trapi2_verdicts"].items():
        assert reason.strip()
        assert GOLDENS["validate"][case] != relay["validate"][case], case
    for case, verdict in GOLDENS["validate"].items():
        if case not in GOLDENS["_trapi2_verdicts"]:
            assert verdict == relay["validate"][case], case


@pytest.mark.parametrize("key", ["merge_ab", "merge_ba", "merge_pipeline_ab"])
def test_merged_goldens_are_valid_trapi2(key):
    """The merged outputs the port must reproduce are TRAPI 2.0 messages."""
    from translator_tom import Response

    Response.from_dict(GOLDENS[key])
    assert validate(GOLDENS[key])


@pytest.mark.parametrize(
    "name",
    [
        "response_aragorn.json",
        "response_arax.json",
        "response_blocklist.json",
        "response_empty.json",
        "response_pathfinder.json",
    ],
)
def test_corpus_responses_are_valid_trapi2(name):
    from translator_tom import Response

    Response.from_dict(load(name))
