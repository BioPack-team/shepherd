...
@pytest.mark.asyncio
async def test_aragorn_lookup_gandalf_lookup_uses_add_task(redis_mock, mocker):
    """When ``parameters.gandalf=True``, the lookup hands off to the gandalf
    stream via add_task instead of hitting the kg retrieval URL."""
    msg = {
        "message": {
            "query_graph": {
                "nodes": {"a": {"ids": ["X:1"]}, "b": {}},
        ...
        "parameters": {"gandalf": True, "timeout": 5},
    }
    mocker.patch(
        "workers.aragorn_lookup.worker.get_message",
        new_callable=mocker.AsyncMock,
    ...
    mocker.patch(
        "workers.aragorn_lookup.worker.save_message",
        new_callable=mocker.AsyncMock,
    )
    mock_add_task = mocker.patch(
        "workers.aragorn_lookup.worker.add_task",
        new_callable=mocker.AsyncMock,
    )
    mocker.patch(
        "workers.aragorn_lookup.worker.get_running_callbacks",
    ...
    await aragorn_lookup(task, logger)
    # The gandalf add_task should have been called with the gandalf stream.
    assert mock_add_task.called
    args, _ = mock_add_task.call_args
    assert args[0] == "gandalf"
    assert args[1]["target"] == "aragorn"
