...
| DEC-10 | **A `kp` list is forwarded to Retriever** (a forward-looking deviation from DEC-1). The intended meaning of `kp` is to filter returned edges to those whose `sources` list the named infores. ARAX does not do that: a user-specified `kp` only chooses which endpoint(s) are queried (`ARAX_expander.py:431-435`). In the port, the query still only goes to Retriever, and the `kp` list (from `expand(kp=…)`, or from the `fill` allowlist via WF-03) is passed along in the Retriever request, so Retriever can apply the filtering. Retriever decides which values are valid, so the port drops ARAX's `valid_kps` / `InvalidKP` check. ARAX's other side effects of a user-specified `kp` are kept per DEC-1: skipping the knowledge-source constraint allowlist/denylist and the blocked-KP list, and marking other KPs "Skipped" in the query plan. (An updated TRAPI schema may later formalize this filtering.) | EXP-01 and WF-03 are ported, and the list is forwarded to Retriever. Open: the Retriever request field and format for the list; see Part F Q2. |
...
2. **KP roster** (decided: DEC-4, queries use Retriever only, which covers
   ...
   DEC-10, a `kp`/`fill` list is forwarded to Retriever; DEC-11, the meta-KG
   comes from Retriever, with ARAX's additions). Still open: which Retriever
   request field and format carry the forwarded `kp` list, and whether
   Retriever supports it yet.
