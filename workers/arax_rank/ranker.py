...
class ARAXRanker:
    ...
    def _build_edge_lookup_and_stats(self, msg: Dict) -> None:
        ...
        self.logger.debug(
            f"Built edge lookup with {len(self.kg_edge_id_to_edge)} edges"
        )
        ...
        if edge.get("attributes"):
            for attr in edge["attributes"]:
                ...
                if original_attr_name is not None:
                    normalized_score = self._normalize_attribute(
                        original_attr_name, value
                    )
                else:
                ...
                if original_attr_name and self.known_attributes_to_trust.get(
                    original_attr_name
                ):
                    if normalized_score > 0:
                        weighted = (
                            normalized_score
                            * self.known_attributes_to_trust[original_attr_name]
                        )
                    ...
                    if normalized_score > 0:
                        weighted = (
                            normalized_score
                            * self.known_attributes_to_trust[attr_type_id]
                        )
                        edge_attribute_score_list.append(weighted)
                elif (
                    attr_type_id == "biolink:publications"
    ...
    def _normalize_chi_square(self, value: float) -> float:
        ...
        return max_value / (
            1 + np.exp(-curve_steepness * (log_value - logistic_midpoint))
        )
    ...
    def _build_result_graph_nx(
        self, qg_nx: nx.MultiDiGraph, result: Dict
    ) -> nx.MultiDiGraph:
        """
        ...
        for qedge_key, edge_info in valid_edge_id_info.items():
            ...
            if averaged_scores:
                ...
                res_graph[qedge_tuple[0]][qedge_tuple[1]][qedge_tuple[2]][
                    "weight"
                ] = combined_score
    ...
    def _score_by_longest_path(self, graph: nx.MultiDiGraph) -> float:
        ...
        try:
            ...
            map_node_name_to_index = {
                node_id: node_index for node_index, node_id in enumerate(graph.nodes)
            }
